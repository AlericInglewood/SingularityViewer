...
struct CurlEasyHandleEvents {
	// Events.
	virtual void added_to_multi_handle(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) = 0;
	virtual void finished(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) = 0;
	virtual void removed_from_multi_handle(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) = 0;
};
...
class AIThreadSafeBufferedCurlEasyRequest;
...
class CurlEasyHandle : public boost::noncopyable, protected CurlEasyHandleEvents {
  ...
  private:
	...
	CURLMcode add_handle_to_multi(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w, CURLM* multi_handle);
	CURLMcode remove_handle_from_multi(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w, CURLM* multi_handle);
...
class CurlEasyRequest : public CurlEasyHandle {
  ...
  public:
	...
	void done(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) { finished(curl_easy_request_w); }
  ...
  private:
	// This class may only be created by constructing a AIThreadSafeCurlEasyRequest.
	friend class AIThreadSafeCurlEasyRequest;
  ...
  public:
	...
	AIThreadSafeCurlEasyRequest* get_lockobj(void);

  protected:
	// Pass events to parent.
	/*virtual*/ void added_to_multi_handle(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) { if (mParent) mParent->added_to_multi_handle(curl_easy_request_w); }
	/*virtual*/ void finished(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) { if (mParent) mParent->finished(curl_easy_request_w); }
	/*virtual*/ void removed_from_multi_handle(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w) { if (mParent) mParent->removed_from_multi_handle(curl_easy_request_w); }
};
...
class CurlResponderBuffer : protected CurlEasyHandleEvents {
  public:
  ...
  protected:
	/*virtual*/ void added_to_multi_handle(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w);
	/*virtual*/ void finished(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w);
	/*virtual*/ void removed_from_multi_handle(AIAccess<AICurlPrivate::CurlEasyRequest>& curl_easy_request_w);
  ...
  private:
	// This class may only be created by constructing a AIThreadSafeBufferedCurlEasyRequest.
	friend class AIThreadSafeBufferedCurlEasyRequest;
  ...
  public:
	// Return pointer to the ThreadSafe (wrapped) version of this object.
	AIThreadSafeBufferedCurlEasyRequest* get_lockobj(void);
};
...
class AIThreadSafeCurlEasyRequest : public AIThreadSafeSimple<CurlEasyRequest> {
  public:
	AIThreadSafeCurlEasyRequest(void) throw(AICurlNoEasyHandle) : mReferenceCount(0)
        { new (ptr()) CurlEasyRequest;
		  Dout(dc::curl, "Creating AIThreadSafeCurlEasyRequest with this = " << (void*)this); }
	virtual ~AIThreadSafeCurlEasyRequest()
	    { Dout(dc::curl, "Destructing AIThreadSafeCurlEasyRequest with this = " << (void*)this); }

  private:
	LLAtomicU32 mReferenceCount;

	friend void intrusive_ptr_add_ref(AIThreadSafeCurlEasyRequest* p);	// Called by boost::intrusive_ptr when a new copy of a boost::intrusive_ptr<AIThreadSafeCurlEasyRequest> is made.
...
// allow casting the underlaying CurlEasyRequest to AIThreadSafeCurlEasyRequest, independent of
// what class it is part of: AIThreadSafeCurlEasyRequest or AIThreadSafeBufferedCurlEasyRequest.
// The virtual destructor of AIThreadSafeCurlEasyRequest allows to treat each easy handle transparently
// as a AIThreadSafeCurlEasyRequest object, or optionally dynamic_cast it to a AIThreadSafeBufferedCurlEasyRequest.
class AIThreadSafeBufferedCurlEasyRequest : public AIThreadSafeCurlEasyRequest, public AIThreadSafeSimple<CurlResponderBuffer> {
  public:
	AIThreadSafeBufferedCurlEasyRequest(void) throw(AICurlNoEasyHandle) { new (AIThreadSafeSimple<CurlResponderBuffer>::ptr()) CurlResponderBuffer; }
};
